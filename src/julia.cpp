...
Color JuliaGenerator::getPixelColor(int x, int y) const {
    // Map screen coordinates (x, y) to complex plane [-2, 2] x [-2, 2]
    ...
    // Simple grayscale-to-color mapping
    if (iterations == maxIterations) {
        return {0, 0, 0}; // Inside the set is black
    }

    // Dynamic color mapping based on escape time
